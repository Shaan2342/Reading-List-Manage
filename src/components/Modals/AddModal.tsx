	...
	return (
		...
		>
			...
			<div className="md:w-[30vw] w-[90%] bg-white rounded-lg shadow-md z-50 flex flex-col items-center gap-3 px-5 py-6">
				...
				<button
					className={`w-full mt-3 rounded-md h-9 ${
						!taskData.title || !taskData.bookName || !taskData.authorName
							? 'bg-gray-300 cursor-not-allowed'
							: 'bg-orange-400 hover:bg-orange-500'
					} text-white font-medium transition-colors`}
					onClick={handleSubmit}
					disabled={!taskData.title || !taskData.bookName || !taskData.authorName}
				>
					Add Book
				</button>
				{(!taskData.title || !taskData.bookName || !taskData.authorName) && (
					<p className="text-sm text-red-500 mt-2">
						Please fill in all required fields (Title, Book Name, Author)
					</p>
				)}
			</div>
		</div>
